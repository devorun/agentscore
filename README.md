...
| **Agent Stack** | Evaluated — not integrated | — | — |
...
- **Agent Stack — evaluated, not integrated.** Circle's [Agent Stack](https://developers.circle.com/agent-stack) (Agent Wallets, Agent Marketplace, Circle CLI, Nanopayments-over-Gateway, Circle Skills) bundles the **same payment primitives we already ship** — Nanopayments, Gateway, and x402 are precisely what `nanopay.ts` integrates and proves on-chain — so that layer is redundant here. Its one genuinely new capability, **policy-controlled Agent Wallets** (on-chain spending guardrails), *does* support Arc Testnet, but it is **CLI-only** (`@circle-fin/cli`) behind an **interactive Circle login + Terms acceptance**, with no programmatic Node SDK to slot into our signer — so it can't be wired into our hands-off, testnet-$0 worker the way the developer-controlled Wallets API can. We already demonstrate a **Circle-custodied agent signing real Arc contract calls** programmatically (`SIGNER_MODE=circle`, job #158772). We document the choice rather than bolt on a login-gated CLI dependency for a marginal delta; the `signer.ts` mode switch leaves the door open to add a policy-controlled Agent-Wallet signer behind an env flag once its login/Terms are completed. Builder feedback below.
...
### Agent Stack — evaluated

- **Why we looked at it.** The Agentic Economy track calls for it and we hadn't evaluated it, so we researched it from the current docs, npm, and the agent setup skill (`agents.circle.com/skills/setup.md`).
- **What overlaps.** Three of its five components — **Nanopayments, Gateway, x402** — are exactly what AgentScore already integrates and proves on-chain, so for us they are redundant.
- **What's genuinely new, and what blocked it.** **Policy-controlled Agent Wallets** (programmable spending guardrails) are a real addition and support Arc Testnet. But wallet setup is **CLI-only** (`@circle-fin/cli`) behind an **interactive Circle login + Terms acceptance**, with **no programmatic Node/TS SDK** — unlike `@circle-fin/developer-controlled-wallets`, which we use. Free-tier / no-card status for Arc Testnet isn't clearly documented. That combination doesn't fit an automated, testnet-only, $0 backend signer.
- **Recommendation.** Ship a programmatic Agent-Wallet + spending-policy **API/SDK** at parity with developer-controlled Wallets, so agents can be provisioned and policied from a backend without an interactive CLI login; document Arc Testnet pricing and any free tier; and clarify how Agent Wallets relate to (or supersede) developer-controlled Wallets so integrators don't build the overlap twice.
