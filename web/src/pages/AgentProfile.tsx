...
function DecayCard({ breakdown: b }: { breakdown: ScoreBreakdown }) {
  ...
  // Display only (the score itself never reads a clock). The reference block is
  // the chain tip whenever history could be read that far (explorer + chain
  // backfill); it trails only if the backfill fell short.
  const historyLagHours = Math.floor((Date.now() / 1000 - b.asOf.timestamp) / 3600)
  return (
    <Card className="flex flex-col gap-3 rounded-xl border-border bg-card p-5">
      ...
      {historyLagHours >= 1 ? (
        <p className="text-[12px] leading-relaxed text-warning">
          Onchain history could only be read up to about {historyLagHours} h ago, so this score stops at that block;
          newer settlements count once they can be read.
        </p>
      ) : null}
